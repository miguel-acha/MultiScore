...
// Rebuilt so the marker itself tells you the RESULT (shape+color from
// lib/outcomes.ts - a goal is a lime ball, a save is a yellow circle, a
// block is a red square, etc.) while the TEAM is read from the lane
// (home above the line, away below) and the crest at that lane's end -
// the old version colored markers by team, which read as "red = missed"
// to a first-time viewer since red also meant "rival" everywhere else.
  ...
  return (
    <div className="clip-menu w-full border border-(--color-border) bg-(--color-surface) px-5 py-4">
      <div className="mb-3 flex items-center gap-2 text-sm font-medium">
        ...
        <span className="ml-auto text-xs uppercase tracking-wide text-(--color-text-faint)">Línea de tiempo</span>
      </div>
      ...
      <div className="relative h-16">
        ...
        {[0, 45, 90].map((m) => (
          <div key={m} className="absolute top-1/2 flex -translate-y-1/2 flex-col items-center" style={{ left: `${(m / maxMinute) * 100}%` }}>
            ...
            <span className="mt-6 text-[10px] text-(--color-text-faint)">{m === 45 ? "ET" : `${m}'`}</span>
          </div>
        ))}
        {sorted.map((s) => {
          ...
          return (
            <button
              ...
              style={{ left: `${leftPct}%`, top: isHome ? "calc(50% - 20px)" : "calc(50% + 8px)" }}
            >
              <span
                className="block rounded-full transition-shadow"
                style={{ padding: isSelected ? 3 : 0, boxShadow: isSelected ? "0 0 0 2px rgba(255,255,255,0.4)" : "none" }}
              >
        ...
        <AnimatePresence>
          {hovered && (
            <motion.div
              ...
              style={{
                left: `${Math.min(92, Math.max(8, ((hovered.minute ?? 0) / maxMinute) * 100))}%`,
                top: hovered.team === match.home_team ? "-8px" : "auto",
                bottom: hovered.team === match.home_team ? "auto" : "-8px",
              }}
            >
              <div
                className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-(--color-surface-3) text-[10px] font-semibold"
              >
                {initials(hovered.player_nickname ?? hovered.player ?? "?")}
              </div>
              <div className="whitespace-nowrap">
                <p className="font-medium">{hovered.player_nickname ?? hovered.player}</p>
                <p className="text-(--color-text-faint)">
                  {hovered.minute}&apos; · {outcomeStyle(hovered.shot_outcome, hovered.is_goal === 1).label} · xG {hovered.xg_full.toFixed(2)}
                </p>
              </div>
            </motion.div>
          )}
